...
function createPost($conn, $post)
{
  ...
  $stmt = pg_execute($conn, "create_post", array($credentials['username'], $hash));
  $post = pg_fetch_assoc($stmt);
...
function tryLogin($conn, $credentials)
{
  ...
  $stmt = pg_prepare($conn, "get_home_posts", 'SELECT * FROM users WHERE username = $1');
  $stmt = pg_execute($conn, "get_home_posts", array($credentials['username']));
